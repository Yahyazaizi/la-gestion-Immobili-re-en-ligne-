...
if(isset($_POST['add']))
{
    ...
    $required = [
        'title', 'content', 'ptype', 'bhk', 'bed', 'balc', 'hall', 'stype', 'bath', 'kitc', 'floor',
        'price', 'city', 'asize', 'loc', 'state', 'status', 'feature', 'totalfl', 'isFeatured'
    ];
    $missing = [];
    ...
    $requiredFiles = ['aimage','aimage1','aimage2','aimage3','aimage4'];
    ...
    if(!empty($missing)) {
        $error = "<p class='alert alert-warning'>Veuillez remplir tous les champs obligatoires.</p>";
    } else {
        try {
			$title=$_POST['title'];
			...
			if($result) {
				...
				throw new Exception("Property Not Inserted. Please check your inputs.");
			}
		} catch (Exception $e) {
			$error="<p class='alert alert-warning'>Error: " . $e->getMessage() . "</p>";
		}
    }
}							
?>
...
/* Pour forcer le style sur les select */
select {
  appearance: none;
...
/* Pour textarea un petit reset (optionnel mais recommandé) */
								...
								<div class="description">
									...
									<?php echo $msg; ?>
										...
										<div class="form-group row">
											...
											<textarea class="tinymce form-control  " name="feature" rows="10" cols="30" type="hidden">
											<?php
$featureContent = '<div class="col-md-4">
<pre>
    <ul><li class="mb-3"><span class="text-secondary font-weight-bold">Property Age : </span>10 Years</li>
...
echo htmlspecialchars(getFeatureText($featureContent)) ;
?>
